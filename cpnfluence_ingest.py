"""
Confluence ingestion script (PAT only) with OpenAI image descriptions.
...
def describe_image_via_openai(image_url):
    ...
    try:
        resp = openai.chat.completions.create(
            ...
            messages=[
                {'role':'user','content':[
                    {'type':'text','text':'Describe this image in detail, including objects, text, charts, and any labels.'},
...
def index_chunk(title, url, space, page_id, chunk_idx, text, images):
    ...
    while True:
        ...
        for page in results:
            ...
            for i,ch in enumerate(chunks):
                index_chunk(title, url, space_key, page_id, i, ch, images)
        start += 25
        if len(results) < 25:
            break
    print('Done')
...
def ingest_all_spaces(max_spaces=50):
    ...
    start = 0
    seen = 0
    while True and seen < max_spaces:
